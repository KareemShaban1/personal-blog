...
<body class="bg-gray-100 font-family-karla flex">
    ...
    <aside class="relative bg-sidebar h-screen w-64 hidden sm:block shadow-xl">
        ...
        <nav class="text-white text-base font-semibold">
            <a href="{{ route('backend.index') }}"
                class="{{ request()->routeIs('backend.index') ? 'active-nav-link' : 'opacity-75 hover:opacity-100' }} flex items-center text-white py-4 pl-6 nav-item">
        ...
        <header class="w-full items-center bg-white py-2 px-6 hidden sm:flex">
            ...
            <div x-data="{ isOpen: false }" class="relative w-1/2 flex justify-end">
                ...
                <div x-show="isOpen" class="absolute w-32 bg-white rounded-lg shadow-lg py-2 mt-16">
                    <a href="{{ route('backend.account.index') }}" class="block px-4 py-2 account-link hover:text-white">Account</a>
    ...
    @if (request()->routeIs('*.page.create') || request()->routeIs('*.page.edit'))
        ...
        <link href="{{asset('backend/css/summernote-lite.min.css')}}" rel="stylesheet">
        <script src="{{asset('backend/js/summernote-lite.min.js')}}"></script>        <script>
            $('#summernote').summernote({
                placeholder: 'Hello ..!',
                tabsize: 2,
                height: 120,
