<x-admin-layout>
    ...
    <div class="w-full h-screen overflow-x-hidden border-t flex flex-col">
        <main class="w-full flex-grow p-6">
            <h1 class="w-full text-3xl text-black pb-6">{{ trans('category_trans.Add_Category') }}</h1>
            ...
            <div class="w-full mt-12">
                ...
                <form method="POST" action="{{ route('backend.category.store') }}">
                    ...
                    <button
                        class="px-4 py-1 text-white font-light tracking-wider bg-blue-600 rounded">{{ trans('category_trans.Add_Category') }}</button>
                </form>
            </div>
        </main>
    </div>
