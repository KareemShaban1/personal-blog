        ...
        <main class="w-full flex-grow p-6">
            ...
            <div class="w-full mt-12">
                <p class="text-xl pb-3 flex items-center">
                    <i class="fas fa-list mr-3"></i> {{ trans('post_trans.Search_Result') }}
                </p>
                @if ($posts->isNotEmpty())
                    <div class="w-full bg-white text-left p-4 mb-2">{{ trans('post_trans.Found') }} {{ $posts->total() }}
                        {{ trans('post_trans.Records_In_Post') }}</div>
                            ...
                            <tbody>
                                @foreach ($posts as $post)
                                    <tr class="hover:bg-grey-lighter">
                                        ...
                                        <td class="py-4 px-6 border-b border-grey-light">{{ $post->category->name }}
                                        </td>
                    ...
                    {!! $posts->links() !!}
                @else
                    <div class="w-full bg-red-500 text-left p-4 mb-2 text-white">Sorry No Results Founded, try again..
                    </div>
                @endif
        </main>
    </div>
