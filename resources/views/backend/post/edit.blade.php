<x-admin-layout>
    ...
    <div class="w-full h-screen overflow-x-hidden border-t flex flex-col">
        <main class="w-full flex-grow p-6">
            <h1 class="w-full text-3xl text-black pb-6">{{ trans('post_trans.Edit_Post') }}</h1>
                    ...
                    <div class="grid gap-6 mb-6 md:grid-cols-2">
                            ...
                            <select
                                ...
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}"
                                        {{ $post->category->name == "$category->name" ? 'selected' : '' }}>
                        ...
                        <div class="mb-1">
                            ...
                            <select
                                ...
                                <option value="1" {{ $post->status == true ? 'selected' : '' }}>
                                    {{ trans('post_trans.Yes') }}</option>
                                <option value="0" {{ $post->status == false ? 'selected' : '' }}>
                                    {{ trans('post_trans.No') }}</option>
                            </select>
                        </div>
                        {{--  --}}
                        <div class="mb-2">
